...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        ...
        <h2 style="font-size:22px; font-weight:700; color:#0F172A; margin:10px 0 0;">Edit Enquiry</h2>
        <p style="font-size:13px; color:#64748B; margin:6px 0 0;">Update enquiry details before saving.</p>
    </div>
</div>
...
<form action="{{ route('admin.enquiries.update', $enquiry->id) }}" method="POST">
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <div>
                ...
                <p style="font-size:12px;color:#94A3B8;margin:2px 0 0;">Edit the enquiry record below.</p>
            </div>
        </div>
        <div class="form-card-body">
            <div style="display:grid; gap:18px;">
                <div>
                    ...
                    <select name="product_id" id="product_id" class="field-select">
                        @foreach($products as $id => $label)
                            <option value="{{ $id }}" {{ (string) old('product_id', $enquiry->product_id) === (string) $id ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $enquiry->name) }}" class="field-input">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px;">
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $enquiry->email) }}" class="field-input">
                        @error('email')<p class="field-error"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="field-label" for="phone">Phone</label>
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $enquiry->phone) }}" class="field-input">
                ...
                <div>
                    ...
                    <input type="text" name="subject" id="subject" value="{{ old('subject', $enquiry->subject) }}" class="field-input">
                ...
                <div style="display:grid; gap:18px; grid-template-columns:1fr 1fr; align-items:start;">
                    ...
                    <div style="display:flex; align-items:center; gap:12px; padding-top:24px;">
                        <label style="display:flex; align-items:center; gap:10px; cursor:pointer; font-size:13px; color:#374151;">
                            <input type="checkbox" name="is_read" value="1" {{ old('is_read', $enquiry->is_read) ? 'checked' : '' }} style="width:auto;">
                            Mark as read
                        </label>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="admin_note" id="admin_note" class="field-textarea">{{ old('admin_note', $enquiry->admin_note) }}</textarea>
    ...
    <div style="margin-top:20px; display:flex; flex-wrap:wrap; gap:12px;">
        <button type="submit" class="btn-primary"><i class="fas fa-save"></i> Update Enquiry</button>
