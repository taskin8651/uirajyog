...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        <a href="{{ route('admin.faqs.index') }}" style="font-size:13px; color:var(--accent); text-decoration:none; font-weight:600;">← Back to FAQs</a>
        <h2 style="font-size:22px; font-weight:700; color:#0F172A; margin:10px 0 0;">Edit FAQ</h2>
        <p style="font-size:13px; color:#64748B; margin:6px 0 0;">Update the frequently asked question and answer.</p>
    </div>
</div>
...
<form action="{{ route('admin.faqs.update', $faq->id) }}" method="POST">
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <div>
                <p style="font-size:14px;font-weight:700;color:#0F172A;margin:0;">FAQ content</p>
                <p style="font-size:12px;color:#94A3B8;margin:2px 0 0;">Edit the FAQ.</p>
            </div>
        </div>
        <div class="form-card-body">
            <div style="display:grid; gap:18px;">
                <div>
                    ...
                    <input type="text" name="question" id="question" value="{{ old('question', $faq->question) }}" class="field-input">
                    @error('question')<p class="field-error"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="field-label" for="answer">Answer</label>
                    <textarea name="answer" id="answer" class="field-textarea">{{ old('answer', $faq->answer) }}</textarea>
                    @error('answer')<p class="field-error"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>@enderror
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px; align-items:start;">
                    <div>
                        <label class="field-label" for="sort_order">Sort Order</label>
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $faq->sort_order) }}" class="field-input">
                    ...
                    <div style="display:flex; align-items:center; gap:12px; padding-top:24px;">
                        <label style="display:flex; align-items:center; gap:10px; cursor:pointer; font-size:13px; color:#374151;">
                            <input type="checkbox" name="status" value="1" {{ old('status', $faq->status) ? 'checked' : '' }} style="width:auto;">
                            Active
                        </label>
                    </div>
                </div>
    ...
    <div style="margin-top:20px; display:flex; flex-wrap:wrap; gap:12px;">
        <button type="submit" class="btn-primary"><i class="fas fa-save"></i> Update FAQ</button>
