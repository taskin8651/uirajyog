...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        <a href="{{ route('admin.about-sections.index') }}" style="font-size:13px; color:var(--accent); text-decoration:none; font-weight:600;">← Back to about sections</a>
        <h2 style="font-size:22px; font-weight:700; color:#0F172A; margin:10px 0 0;">Edit About Section</h2>
        <p style="font-size:13px; color:#64748B; margin:6px 0 0;">Update the about section content and image.</p>
    </div>
</div>
...
<form action="{{ route('admin.about-sections.update', $aboutSection->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <div>
                <p style="font-size:14px;font-weight:700;color:#0F172A;margin:0;">About section content</p>
                <p style="font-size:12px;color:#94A3B8;margin:2px 0 0;">Edit the about section.</p>
            </div>
        </div>
        <div class="form-card-body">
            <div style="display:grid; gap:18px;">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $aboutSection->title) }}" class="field-input">
                    @error('title')<p class="field-error"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="field-label" for="description">Description</label>
                    <textarea name="description" id="description" class="field-textarea">{{ old('description', $aboutSection->description) }}</textarea>
                ...
                <div>
                    ...
                    @if($aboutSection->image)
                        <div class="image-actions">
                            <img src="{{ $aboutSection->image->getUrl() }}" alt="Current image" class="image-preview">
                            <form action="{{ route('admin.about-sections.image.destroy', $aboutSection->id) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px; align-items:start;">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $aboutSection->sort_order) }}" class="field-input">
                    ...
                    <div style="display:flex; align-items:center; gap:12px; padding-top:24px;">
                        <label style="display:flex; align-items:center; gap:10px; cursor:pointer; font-size:13px; color:#374151;">
                            <input type="checkbox" name="status" value="1" {{ old('status', $aboutSection->status) ? 'checked' : '' }} style="width:auto;">
                            Active
                        </label>
                    </div>
                </div>
    ...
    <div style="margin-top:20px; display:flex; flex-wrap:wrap; gap:12px;">
        ...
        <a href="{{ route('admin.about-sections.index') }}" class="btn-ghost">Cancel</a>
    </div>
</form>
@endsection
