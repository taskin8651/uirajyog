...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        ...
        <h2 style="font-size:22px; font-weight:700; color:#0F172A; margin:10px 0 0;">Edit Hero Section</h2>
        <p style="font-size:13px; color:#64748B; margin:6px 0 0;">Update the hero section content and image.</p>
    </div>
</div>
...
<form action="{{ route('admin.hero-sections.update', $heroSection->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <div>
                <p style="font-size:14px;font-weight:700;color:#0F172A;margin:0;">Hero section content</p>
                <p style="font-size:12px;color:#94A3B8;margin:2px 0 0;">Edit the homepage hero section.</p>
            </div>
        </div>
        <div class="form-card-body">
            <div style="display:grid; gap:18px;">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $heroSection->title) }}" class="field-input">
                ...
                <div>
                    ...
                    <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $heroSection->subtitle) }}" class="field-input">
                    @error('subtitle')<p class="field-error"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="field-label" for="description">Description</label>
                    <textarea name="description" id="description" class="field-textarea">{{ old('description', $heroSection->description) }}</textarea>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px;">
                    <div>
                        ...
                        <input type="text" name="button_text" id="button_text" value="{{ old('button_text', $heroSection->button_text) }}" class="field-input">
                        @error('button_text')<p class="field-error"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="field-label" for="button_link">Primary Button Link</label>
                        <input type="text" name="button_link" id="button_link" value="{{ old('button_link', $heroSection->button_link) }}" class="field-input">
                        @error('button_link')<p class="field-error"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>@enderror
                    </div>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px;">
                    <div>
                        <label class="field-label" for="secondary_button_text">Secondary Button Text</label>
                        <input type="text" name="secondary_button_text" id="secondary_button_text" value="{{ old('secondary_button_text', $heroSection->secondary_button_text) }}" class="field-input">
                        @error('secondary_button_text')<p class="field-error"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="field-label" for="secondary_button_link">Secondary Button Link</label>
                        <input type="text" name="secondary_button_link" id="secondary_button_link" value="{{ old('secondary_button_link', $heroSection->secondary_button_link) }}" class="field-input">
                        @error('secondary_button_link')<p class="field-error"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>@enderror
                    </div>
                </div>
                <div>
                    ...
                    @if($heroSection->image)
                        <div class="image-actions">
                            <img src="{{ $heroSection->image->getUrl() }}" alt="Current image" class="image-preview">
                            <form action="{{ route('admin.hero-sections.image.destroy', $heroSection->id) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px; align-items:start;">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $heroSection->sort_order) }}" class="field-input">
                    ...
                    <div style="display:flex; align-items:center; gap:12px; padding-top:24px;">
                        <label style="display:flex; align-items:center; gap:10px; cursor:pointer; font-size:13px; color:#374151;">
                            <input type="checkbox" name="status" value="1" {{ old('status', $heroSection->status) ? 'checked' : '' }} style="width:auto;">
                            Active
                        </label>
                    </div>
                </div>
    ...
    <div style="margin-top:20px; display:flex; flex-wrap:wrap; gap:12px;">
        <button type="submit" class="btn-primary"><i class="fas fa-save"></i> Update Hero Section</button>
        <a href="{{ route('admin.hero-sections.index') }}" class="btn-ghost">Cancel</a>
    </div>
</form>
@endsection
