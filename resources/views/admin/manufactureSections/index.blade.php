...
@section('page-title', 'Manufacture Sections')
...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        <h2 style="font-size:22px; font-weight:700; color:#0F172A; margin:0;">Manufacture Sections</h2>
        <p style="font-size:13px; color:#64748B; margin:6px 0 0;">Manage manufacture section content and images.</p>
    </div>
    <a href="{{ route('admin.manufacture-sections.create') }}" class="btn-primary">
...
<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(160px,1fr)); gap:14px; margin-bottom:24px;">
    <div style="background:#fff; border:1px solid #E2E8F0; border-radius:12px; padding:18px;">
        ...
        <p style="font-size:26px; font-weight:700; color:#0F172A; margin:0;">{{ $manufactureSections->total() }}</p>
    </div>
    <div style="background:#fff; border:1px solid #E2E8F0; border-radius:12px; padding:18px;">
        <p style="font-size:11px; font-weight:700; color:#94A3B8; text-transform:uppercase; margin:0 0 8px;">Active</p>
        <p style="font-size:26px; font-weight:700; color:#0F172A; margin:0;">{{ $manufactureSections->where('status', 1)->count() }}</p>
    </div>
</div>
...
<div class="page-card">
    <div style="padding:18px 22px; border-bottom:1px solid #F1F5F9; display:flex; align-items:center; justify-content:space-between; gap:10px; flex-wrap:wrap;">
        <p style="font-size:14px; font-weight:700; color:#0F172A; margin:0;">Manufacture section listings</p>
        <span style="font-size:12px; color:#94A3B8;">Showing {{ $manufactureSections->count() }} of {{ $manufactureSections->total() }}</span>
    </div>
    <div style="overflow-x:auto; padding:10px;">
        <table style="width:100%; border-collapse:collapse; min-width:720px;">
            ...
            <tbody>
                @forelse($manufactureSections as $section)
                <tr style="border-bottom:1px solid #F1F5F9;">
                    <td style="padding:14px 16px; vertical-align:middle;">
                        @if($section->image)
                            <img src="{{ $section->image->getUrl() }}" alt="Manufacture image" class="image-thumb">
                        @else
                    ...
                    <td style="padding:14px 16px; vertical-align:middle; text-align:right;">
                        <a href="{{ route('admin.manufacture-sections.show', $section->id) }}" class="btn-ghost" style="margin-right:6px;">View</a>
                        <a href="{{ route('admin.manufacture-sections.edit', $section->id) }}" class="btn-ghost">Edit</a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="5" style="padding:18px 16px; color:#64748B; text-align:center;">No manufacture sections found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div style="margin-top:18px;">{{ $manufactureSections->links() }}</div>
@endsection